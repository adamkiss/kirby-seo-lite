...
class Meta
{
	...
	protected function metaArray(): array
	{
		...
		// Robots
		if ($robotsActive = option('adamkiss.seo-lite.robots.active')) {
		...
		if (option('adamkiss.seo-lite.twitter', true)) {
	...
	{
		$cascade = option('adamkiss.seo-lite.cascade');
		if (count(array_intersect(get_class_methods($this), $cascade)) !== count($cascade)) {
			throw new InvalidArgumentException('[kirby-seo] Invalid cascade method in config. Please check your options for `adamkiss.seo-lite.cascade`.');
		}
	...
	protected function fields(string $key): Field|null
	{
		if (($field = $this->page->content($this->lang)->get($key))) {
			if (Str::contains($key, 'robots') && !option('adamkiss.seo-lite.robots.pageSettings')) {
				return null;
			}
	...
	{
		if ($option = option("adamkiss.seo-lite.default.{$key}")) {
	...
	{
		if ($custom = option('adamkiss.seo-lite.dateFormat')) {
	...
	{
		$robots = [];
		foreach (option('adamkiss.seo-lite.robots.types') as $type) {
